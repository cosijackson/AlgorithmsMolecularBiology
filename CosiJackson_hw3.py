...
for c1 in 'ACTGU':
    default_score[c1]={}
    for c2 in 'ACTGU':
        if c1==c2: default_score[c1][c2]=0
        else: default_score[c1][c2]=1

# this also is a global variable as specified in the homework.
special_score = {c:{} for c in 'ACTG-'}
special_score['A'] = {'A':5 , "C": -1, "T": -1, "G": 0, '-': -2}
special_score['C'] = {'A':-1 , "C": 5, "T": 0 , "G": -1, '-': -2}
special_score['T'] = {'A':-1 , "C": 0, "T": 5, "G": -1, '-': -2}
special_score['G'] = {'A':0 , "C": -1, "T": -1, "G":5 , '-':-2 }
special_score['-'] = {'A':-2 , "C": -2, "T":-2 , "G":-2 , '-':0 }
...
def usage():
    ...
    print '\npython hw3.py -F <filename> -L <size of k-mer> \n'
...
#    seq_name and seq. Then it goes line by line through the file only yielding an answer once it has gotten the name and seq. This 
...
def parse_file(file_handler):
    ...
    for line in file_handler:
        ...
        if line.startswith(">"):
            ...
            #On first readline this function fails the if seq_name. writes name to seq_name and then fills in the seq by reading lines
...
def global_dp_edit(seq1,seq2, score=default_score):
    ...
    # first, lets initialize our edit and backpointer matricies.
    ...
    # respectively.
    edit[0] = [i for i,_ in enumerate(edit[0])]
    for i,row in enumerate(edit):
        row[0] = i

    # next up is back. The top row of back is all left pointers (except 0,0)
    ...
    for i in range(1,len(edit)):
        for j in range(1,len(edit[0])):
            ...
            left_score = edit[i][j-1] + 1 
            up_score = edit[i-1][j] + 1

            # find the minimum and store it as our current value
            edit[i][j] = min([diagonal_score,left_score,up_score])
            # figure out which one is the min and store that as the backpointer.
            if edit[i][j] == diagonal_score:
                back[i][j] = 'D'
            elif edit[i][j] == left_score:
                back[i][j] = 'L'
            else:
                back[i][j] = 'U'
...
def main():
    ...
    for i in range(0, len(array_of_seq)-1, 2):
        seq1, seq2, edit_distance = global_dp_edit(array_of_seq[i], array_of_seq[i+1])
    ...
    for i in array_of_seq:
        for j in array_of_seq:
            seq1, seq2, edit_distance = global_dp_edit(i, j)
            answer.append( round(float(len(seq1)-edit_distance)/float(len(seq1)), 5) )
